...
export const FooterLinks = ({ section }: FooterLinksProps) => {
  return (
    <div className="space-y-md-6">
      <h4 className="md-title-large text-md-on-surface font-semibold">{section.title}</h4>
      <ul className="space-y-md-3">
        {section.links.map((link, index) => (
          <li key={index}>
            <a
              href={link.href}
              className="group md-body-medium text-md-on-surface-variant hover:text-md-primary transition-all duration-md-medium2 flex items-center gap-md-2"
            >
